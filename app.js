...
async function saveMessageToSaved(message) {
  if (!supabaseClient || !currentUser || !message?.body) return;
  const { error } = await supabaseClient.from("saved_messages").insert({
...
async function loadSavedMessages() {
  ...
  if (error) {
    ...
    list.innerHTML = `<div class="welcome"><h2>خطا</h2><p>پیام‌های ذخیره‌شده بارگذاری نشد.</p></div>`;
    return;
  }
  if (!data?.length) {
    list.innerHTML = `<div class="welcome"><h2>پیام ذخیره‌شده‌ای نیست</h2><p>پیام‌های مهم خود را اینجا نگه دارید.</p></div>`;
    return;
  }
  list.innerHTML = data.map(item => `
    <div class="saved-message-item" data-saved-id="${escapeHtml(item.id)}">
      <div class="saved-content">${escapeHtml(item.content)}</div>
      <button type="button" class="saved-message-delete" data-delete-saved="${escapeHtml(item.id)}" aria-label="حذف">×</button>
    </div>
  `).join("");
}
...
async function deleteSavedMessage(id) {
  const { error } = await supabaseClient
    .from("saved_messages").delete()
    .eq("id", id).eq("user_id", currentUser.id);
  if (error) {
    showToast("حذف پیام انجام نشد.");
    return;
  }
...
function bindEvents() {
  ...
  $("savedMessagesList")?.addEventListener("click", async (event) => {
    ...
    if (deleteId) { await deleteSavedMessage(deleteId); }
  });

  $("messages")?.addEventListener("click", async (event) => {
    const btn = event.target.closest("[data-save-message]");
    if (!btn) return;
    const id = btn.dataset.saveMessage;
